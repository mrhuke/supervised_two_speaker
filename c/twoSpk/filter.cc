...
void CFilter::hammingBandPass(double wc, double wn)
{
	double step, k, tim;
...
void CFilter::hammingDiffOneBandPass(double wc, double wn)
{
	double tim, step;
...
void CFilter::hammingDiffTwoBandPass(double wc, double wn)
{
	double tim, step;
...
CFilter::CFilter(double ws, double wn, double ts, int sampF, int type)
{
	fs = sampF;
...
void CFilter::filtering(double *input, double *output, int sigLength)
{
	int nB, m, n, tim;
...
void CFilter::kaiserPara(double delta, double transBw)
{
	double a, len;
	...
	if (a <= 21) beta = 0;
	else if (a<= 50) beta = 0.5842 * pow((double)a-21, (double)0.4) + 0.07889 * (a-21);
...
void CFilter::kaiserLowPass(double wn)
{
	int tim, step;
	double k, sum;
...
void CFilter::diffKaiserLowPass(double wn)
{
